...
function updateFlyingBubble() {
  ...
  // Wall collision
  if (
    flyingBubble.x - flyingBubble.radius < 0 ||
    flyingBubble.x + flyingBubble.radius > canvas.width
  ) {
    flyingBubble.dx *= -1;
    // Ensure it's inside bounds after bounce
    if (flyingBubble.x - flyingBubble.radius < 0)
      flyingBubble.x = flyingBubble.radius;
    if (flyingBubble.x + flyingBubble.radius > canvas.width)
      flyingBubble.x = canvas.width - flyingBubble.radius;
  }
    ...
    for (let c = 0; c < GRID_COLS; c++) {
      ...
      if (staticBubble && staticBubble.visible) {
        const dist = Math.hypot(
          flyingBubble.x - staticBubble.x,
          flyingBubble.y - staticBubble.y
        );
        if (dist < BUBBLE_DIAMETER - 5) {
          // -5 for a bit of overlap tolerance
...
function snapBubbleToGrid(bubble, collidedWith = null) {
  ...
  for (let rOffset = -1; rOffset <= 1; rOffset++) {
    for (let cOffset = -1; cOffset <= 1; cOffset++) {
      ...
      let approxGridCol = Math.floor(
        (bubble.x - (currentIsOdd ? BUBBLE_RADIUS : 0)) / BUBBLE_DIAMETER
      );
      ...
      if (
        ...
        r >= GRID_ROWS ||
        c < 0 ||
        c >= (isOddTargetRow ? GRID_COLS - 1 : GRID_COLS)
      )
        continue;
  ...
  if (!bestPos) {
    ...
    if (!grid[pos.row][pos.col]) {
      ...
      if (!grid[r][pos.col]) {
        ...
        // Failsafe: if absolutely no spot, might cause overlap or error
        ...
        if (bubble.y - BUBBLE_RADIUS <= 0) {
          ...
          const { x: targetX, y: targetY } = getCanvasCoordinates(
            fallbackPos.row,
            fallbackPos.col
          );
  ...
  if (bestPos) {
    grid[bestPos.r][bestPos.c] = Bubble(
      bestPos.x,
      bestPos.y,
      bubble.color,
      true,
...
function getNeighbors(r, c) {
  ...
  for (const dir of directions) {
    ...
    if (
      ...
      nr < GRID_ROWS &&
      nc >= 0 &&
      nc < (nr % 2 !== 0 ? GRID_COLS - 1 : GRID_COLS)
    ) {
...
document.addEventListener("keydown", (event) => {
  if (event.code === "Space" && !gameOver) {
    ...
    if (paused) {
      audio.pause(); // Pause audio when game is paused
      // Optional: Stop the game loop explicitly if needed
...
function startGame() {
  ...
  if (!gameLoopRequestId) {
    // Prevent multiple loops if restart is called rapidly
    gameLoop();
  }
}
